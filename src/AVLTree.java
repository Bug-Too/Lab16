...
public class AVLTree<E extends Comparable<? super E>> implements Tree<E> {
    ...
    public Iterator<E> iterator() {
        ...
        return new Iterator<E>(){
            public Stack<TreeNode<E>> nodeStack = new Stack();
            public boolean flagFirstTime = true;
            @Override
            public boolean hasNext() {
                //Assign root
                if(nodeStack.isEmpty() && flagFirstTime) {
                    nodeStack.push(root);
                    flagFirstTime = false;
                }
            ...
            @Override
            public E next() {
                //Assign root เพื่อความสบายใจเผื่อใช้ next ก่อน
                if(nodeStack.isEmpty() && flagFirstTime) {
                    nodeStack.push(root);
                    flagFirstTime = false;
                }

                //return null when all done
